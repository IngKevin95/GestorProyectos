...
async def client():
    ...
    async with AsyncClient(base_url="http://localhost:8000", timeout=10.0) as ac:
        yield ac
=======
"""Pytest configuration for total_effortkend tests"""
import pytest
from httpx import AsyncClient
...
total_effortkend_root = Path(__file__).parent.parent
if str(total_effortkend_root) not in sys.path:
    sys.path.insert(0, str(total_effortkend_root))
...
async def setup_test_user():
    ...
    INSERT INTO projects (id, user_id, name, total_effort, planned_effort, completed_effort, responsable, status, state, health_status, priority_strategy, priority_constant, business_value, version, created_at, updated_at)
    VALUES
      ('aaaaaaaa-aaaa-aaaa-aaaa-000000000001'::uuid, '{TEST_USER_ID}'::uuid, 'Project 1', 100000, 0, 0, 'Lead 1', 'Activo', 'ACTIVE', 'ok', 'relative', 0, 50, 0, NOW(), NOW()),
      ('aaaaaaaa-aaaa-aaaa-aaaa-000000000002'::uuid, '{TEST_USER_ID}'::uuid, 'Project 2', 200000, 0, 0, 'Lead 2', 'Activo', 'ACTIVE', 'ok', 'absolute', 80, 75, 0, NOW(), NOW()),
      ('aaaaaaaa-aaaa-aaaa-aaaa-000000000003'::uuid, '{TEST_USER_ID}'::uuid, 'Project 3', 150000, 0, 0, 'Lead 3', 'Activo', 'ACTIVE', 'ok', 'mixed', 50, 60, 0, NOW(), NOW()),
      ('aaaaaaaa-aaaa-aaaa-aaaa-000000000004'::uuid, '{TEST_USER_ID}'::uuid, 'Project 4', 250000, 0, 0, 'Lead 4', 'Activo', 'ACTIVE', 'ok', 'relative', 0, 80, 0, NOW(), NOW()),
      ('aaaaaaaa-aaaa-aaaa-aaaa-000000000005'::uuid, '{TEST_USER_ID}'::uuid, 'Project 5', 175000, 0, 0, 'Lead 5', 'Activo', 'ACTIVE', 'ok', 'absolute', 90, 70, 0, NOW(), NOW());
    """
